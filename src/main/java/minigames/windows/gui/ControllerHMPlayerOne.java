...
public class ControllerHMPlayerOne {
    private static final Logger logger = LogManager.getLogger(ControllerHMPlayerOne.class);
    ...
    public ControllerHMPlayerOne(UserGameData data) {
        this.data = data;
    }
    ...
    private void ValidateMWCharacter_ButtonPressed(ActionEvent event) throws Exception {
        if (event.getSource() == Button_ValidateMWCharacter) {
            if ((TextField_MWCharacter.getText().length() == 1) &&
                    ((TextField_MWCharacter.getText().charAt(0) >= 65 && TextField_MWCharacter.getText().charAt(0) <= 90) || // Uppercase characters
                            (TextField_MWCharacter.getText().charAt(0) >= 97 && TextField_MWCharacter.getText().charAt(0) <= 122))) { //lowercase characters
                ...
                if (data.getHmGameLogic().getCurrentLevel() >= Hangman.getLevelEnd() || !data.getHmGameLogic().getResultWord().contains("_")) {
                    ControllerHMSinglePlayerEndScene myController = new ControllerHMSinglePlayerEndScene(data);
                    FXMLLoader root = new FXMLLoader(getClass().getClassLoader().getResource("DialogHMSinglePlayerEndScene.fxml"));
